...
// OBS: TODOS OS ITENS DESSE ARQUIVO DE TEST FORAM IMPLEMENTADOS PELA TRYBE.

describe('1 - Implemente a função `average`', () => { // teste fornecido pela escola Trybe, nessa questão eu desenvolvi a average.js
  ...
  it('Verifica se ao receber um array que contém valores não numéricos a função `average` retorna `undefined`', () => { // teste fornecido pela escola Trybe, nessa questão eu desenvolvi a average.js
  ...
  it('Verifica se ao receber um array vazio a função `average` retorna `undefined`', () => { // teste fornecido pela escola Trybe, nessa questão eu desenvolvi a average.js
    expect(average([])).toBeUndefined();
  });
});
